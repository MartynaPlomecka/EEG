...
for i=1:length(d) %loop over all subjects
    if d(i).isdir
        ...
        try
     %tu zmienilam zakres epoch, wczesniej bylo miedzy 11 i l saccade, a tu
     %jak widac
        data_pro_right{i} = my_pop_epoch(FullEEG,'40', '11');
        data_pro_left{i} = my_pop_epoch(FullEEG,'40', '10');
        data_anti_right{i} = my_pop_epoch(FullEEG,'41', '13');
        data_anti_left{i} = my_pop_epoch(FullEEG,'41', '12');
